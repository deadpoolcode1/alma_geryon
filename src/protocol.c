...
static void proto_req_cmd_set_drv_pulse_mode_handler(uint8_t *buffer, uint8_t len);
...
static void proto_req_cmd_set_fan_operation_handler(uint8_t *buffer, uint8_t len);
...
struct proto_req_cmd_info proto_req_cmd_list[PROTO_REQ_CMD_MAX] = {
    {.id = PROTO_REQ_CMD_SET_FREQ, .cmd = 'S', .sub_cmd = 'F', .handler = proto_req_cmd_set_freq_handler},
    {.id = PROTO_REQ_CMD_GET_FREQ, .cmd = 'G', .sub_cmd = 'F', .handler = proto_req_cmd_get_freq_handler},
    {.id = PROTO_REQ_CMD_SET_DUTY, .cmd = 'S', .sub_cmd = 'D', .handler = proto_req_cmd_set_duty_handler},
    {.id = PROTO_REQ_CMD_GET_DUTY, .cmd = 'G', .sub_cmd = 'D', .handler = proto_req_cmd_get_duty_handler},
    {.id = PROTO_REQ_CMD_SET_CURRENT, .cmd = 'S', .sub_cmd = 'C', .handler = proto_req_cmd_set_current_handler},
    {.id = PROTO_REQ_CMD_GET_CURRENT, .cmd = 'G', .sub_cmd = 'C', .handler = proto_req_cmd_get_current_handler},
    {.id = PROTO_REQ_CMD_SET_OPERATION, .cmd = 'S', .sub_cmd = 'M', .handler = proto_req_cmd_set_operation_handler},
    {.id = PROTO_REQ_CMD_SET_DRV_PULSE_MODE, .cmd = 'S', .sub_cmd = 'P', .handler = proto_req_cmd_set_drv_pulse_mode_handler},
    {.id = PROTO_REQ_CMD_SET_SYNC_FREQ, .cmd = 'S', .sub_cmd = 'S', .handler = proto_req_cmd_set_sync_freq_handler},
    {.id = PROTO_REQ_CMD_SET_JITTER, .cmd = 'S', .sub_cmd = 'J', .handler = proto_req_cmd_set_jitter_handler},
    {.id = PROTO_REQ_CMD_SET_FAN_OPERATION, .cmd = 'C', .sub_cmd = 'F', .handler = proto_req_cmd_set_fan_operation_handler},
    {.id = PROTO_REQ_CMD_SET_KEEP_ALIVE, .cmd = 'C', .sub_cmd = 'K', .handler = proto_req_cmd_set_keep_alive_handler},
    {.id = PROTO_REQ_CMD_GET_ERROR, .cmd = 'E', .sub_cmd = 'E', .handler = proto_req_cmd_get_error_handler},
    {.id = PROTO_REQ_CMD_GET_VERSION, .cmd = 'C', .sub_cmd = 'V', .handler = proto_req_cmd_get_version_handler},
};

enum protocol_stage_e {
    PROTO_STAGE_WAIT_START,
    PROTO_STAGE_RECEVING
};
...
enum protocol_status_e protocol_accum_frame(struct protocol_info* pInfo, const uint8_t ch)
{
...
void protocol_thread_handler(void) {
    ...
    while(1) {
        ...
        const enum protocol_status_e wait_status = protocol_accum_frame(&g_proto_info, c);
...
void protocol_init(void) {
	...
	thread_protocol_id = k_thread_create(&protocol_thread, app_stack, K_KERNEL_STACK_SIZEOF(app_stack),
