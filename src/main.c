...
void jitter_work_handler(struct k_work* work) {
  ...
  LOG_INF("Freq %d (%d) %d (%d) %d (%d)", freq_sync1, new_psc_1, freq_sync2, new_psc_2, freq_sync3, new_psc_3);
...
static int cmd_set_freq(const struct shell *shell, size_t argc, char **argv)
{
  ...
  htim2.Instance->PSC = new_psc_2;
  htim3.Instance->PSC = new_psc_3;
  LOG_INF("Freq %d (%d) %d (%d) %d (%d)", new_freq, new_psc_1, new_freq, new_psc_2, new_freq, new_psc_3);
...
{
  shell_print(shell, "Frequency %d - Pulse %d - Jitter %d", current_freq, current_pulse, current_jitter);
	return 0;
}
